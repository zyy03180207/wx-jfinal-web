		...
		<script type="text/html" id="tpl">
			...
			<tr>
				...
				<td>
					...
					<a href="javascript:;" data-id="{{ item.id }}" data-opt="upd" class="layui-btn layui-btn-normal layui-btn-mini">初始密码</a>
				</td>
			</tr>
			{{# }); }}
		</script>
					...
					complate: function() { //完成的回调
						...
						$('#content').children('tr').each(function() {
							...
							$that.children('td:last-child').children('a[data-opt=edit]').on('click', function() {
								$.get('admin/adminEdit?id=' + $(this).data('id'), null, function(form) {
							...
							$that.children('td:last-child').children('a[data-opt=upd]').on('click', function() {
								...
								layer.confirm('确认初始化密码吗，此操作会将登录密码修改为【12345678】？', {
									  btn: ['确认','取消'] //按钮
										...
										layerTips.msg("dsd", {icon: 6});
										$.ajax({
											type:"POST",
											url:"admin/adminUpPass",
											dataType:"json",
											data: {"id":id},
											success:function(data) {
												if(data.succ) {
